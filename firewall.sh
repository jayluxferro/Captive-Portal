...
$IPTABLES -t nat -A PREROUTING -m mark --mark 99 -p tcp --dport 80 -j DNAT --to-destination $PORTAL
...
$IPTABLES -A FORWARD -i $INTERNET -o $LAN1 -m state --state ESTABLISHED,RELATED -j ACCEPT
